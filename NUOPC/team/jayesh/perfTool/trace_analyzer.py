...
class TraceAnalyzer(object):
  ...
  ####################################################
  # Read all trace files contained in directories named "traceout*"
  # inside the directory, exe_dir_name
  # The trace files are generated by NUOPC/ESMF in a directory 
  # named "traceout"
  ####################################################
  ...
  ####################################################
  # Get total average overhead (value)
  # FIXME: Not currently used (but retained for future use)
  ####################################################
  def get_tot_avg_overhead(self):
    return self._total_avg_overhead

  ####################################################
  # Get total average overhead (string)
  # FIXME: Not currently used (but retained for future use)
  ####################################################
  ...
  ####################################################
  # Print statistics from the trace file
  # FIXME: Currenly we only print statistics for the last trace file
  ####################################################
  def print_statistics(self, opt="none"):
    self.print_trace(opt)

  ####################################################
  # Util function to update per-PET statistics
  ####################################################
  ...
  ####################################################
  # Util function to update overall statistics
  ####################################################
  ...
  ####################################################
  # Read a single trace in trace_dir_name
  ####################################################
  ...
  # opt = none (No statistics), all (all statistics)
  #       pet (per-PET only), overall (overall only)
  ####################################################
