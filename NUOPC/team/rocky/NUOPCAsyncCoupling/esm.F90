module ESM
    ...
    use NUOPC_Connector, only: cplSS => SetServices, NUOPC_ConnectorGet
...
contains
    ...
    subroutine SetServices(driver, rc)
        ...
        call NUOPC_CompSetInternalEntryPoint(driver, ESMF_METHOD_INITIALIZE, &
            phaseLabelList=(/"IPDv04p7"/), userRoutine=DataInitialize, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
    ...
    subroutine SetModelServices(driver, rc)
        ...
        ! SetServices for ATM
        call NUOPC_DriverAddComp(driver, "ATM", atmSS, comp=child, petList=(/0,1/), rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
        ...
        ! SetServices for OCN
        call NUOPC_DriverAddComp(driver, "OCN", ocnSS, comp=child, petList=(/2,3/), rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
        ...
        ! SetServices for LND
        call NUOPC_DriverAddComp(driver, "LND", lndSS, comp=child, petList=(/4,5/), rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
...
#define WITHCONNECTORS
    ...
    subroutine DataInitialize(driver, importState, exportState, clock, rc)
        type(ESMF_GridComp)  :: driver
        ...
        type(ESMF_State)       :: atmImport, lndExport
        type(ESMF_RouteHandle) :: rh

        rc = ESMF_SUCCESS

        ! By now the LND->ATM connector has generated its
        ! routehandle.  Here we pull it out and put it into
        ! the ATM and LND states so that it can be accessed
        ! there and used to call ESMF_FieldRegrid where
        ! the data is needed.

        call NUOPC_DriverGetComp(driver, "ATM", comp, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) return  ! bail out

        call NUOPC_ModelGet(comp, importState=atmImport, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) return  ! bail out

        call NUOPC_DriverGetComp(driver, "LND", comp, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) return  ! bail out

        call NUOPC_ModelGet(comp, exportState=lndExport, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, file=__FILE__)) return  ! bail out


        call NUOPC_DriverGetComp(driver, srcCompLabel="LND", &
            dstCompLabel="ATM", comp=conn, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, file=__FILE__)) return  ! bail out

        call NUOPC_ConnectorGet(conn, rh=rh, rc=rc)
        ! will fail on OCN PETs since the connector is not defined there
        if (rc == ESMF_SUCCESS) then

            call ESMF_RouteHandleSet(rh, name="lnd2atm_rh", rc=rc )
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, &
                file=__FILE__)) &
                return  ! bail out

            call ESMF_StateAdd(atmImport, (/rh/), rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, &
                file=__FILE__)) &
                return  ! bail out

            call ESMF_StateAdd(lndExport, (/rh/), rc=rc)
            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
                line=__LINE__, &
                file=__FILE__)) &
                return  ! bail out
