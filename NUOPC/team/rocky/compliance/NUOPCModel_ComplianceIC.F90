...
module NUOPCModel_ComplianceIC
  ...
  character(*), parameter :: &
    phaseLabels_Advertise = "IPDv00p1|IPDv01p1|IPDv02p1|IPDv03p1|IPDv04p1"
  character(*), parameter :: &
    phaseLabels_Realize = "IPDv00p2"  ! TODO: deal with other versions of realize
    ...
    if (phaseCount == 0) then
      ...
      do phase=1, phaseCount
        ...
        call NUOPC_GridCompSearchPhaseMapByIndex(comp, ESMF_METHOD_INITIALIZE, &
            phase, phaseLabel, rc)
        if (index(phaseLabels_Advertise, trim(phaseLabel)) > 0) then
            !print *, "Found advertise: "//phaseLabel, "phase=", phase
            regAdvertise = .true.
        endif
        if (index(phaseLabels_Realize, trim(phaseLabel)) > 0) then
            regRealize = .true.
        endif
...
!   !   Initialization routine.
  ...
  recursive subroutine ic_init(comp, importState, exportState, clock, rc)
    ...
    ! get label associate with this phase
    !call NUOPC_GridCompSearchPhaseMapByIndex(comp, &
    !    ESMF_METHOD_INITIALIZE, phase, currentPhaseLabel, rc)
    !print *, "currentPhaseLabel = |" // currentPhaseLabel // "|"
    !if (ESMF_LogFoundError(rc, &
    !  line=__LINE__, &
    !  file=FILENAME)) &
    !  return  ! bail out
...
!-------------------------------------------------------------------------
  ...
  ! Covers: IPDv00p1, IPDv01p1, IPDv02p1, IPDv03p1, IPDv04p1
  !
  recursive subroutine checkPhaseEpilogue_Advertise(prefix, comp, &
    ...
    call checkStateFieldMetadataForAdvertise(prefix, &
  ...
  end subroutine checkPhaseEpilogue_Advertise
...
!            deallocate(fields)
  ...
  recursive subroutine checkStateAttribute(prefix, state, attributeName, &
   ...
   recursive subroutine checkStateFieldMetadataForAdvertise(prefix, &
    ...
    print *, "Inside checkStateFieldMetadataForAdvertise()"
        ...
        do item=1, itemCount
          ...
          if (stateitemtypeList(item) == ESMF_STATEITEM_FIELD) then
            ...
            if (ESMF_LogFoundError(rc, &
              ...
              file=FILENAME)) &
              return  ! bail out
            call checkFieldMetadataForAdvertise(prefix, field=field, rc=rc)
            if (ESMF_LogFoundError(rc, &
              line=__LINE__, &
              file=FILENAME)) &
              return  ! bail out
...
!-------------------------------------------------------------------------
  ...
  recursive subroutine checkFieldMetadataForAdvertise(prefix, field, rc)
    character(*), intent(in)              :: prefix
    type(ESMF_Field)                      :: field
    integer,      intent(out), optional   :: rc
