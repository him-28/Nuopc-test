...
module nuopcExplorerDriver
  ...
  use NUOPC_Compliance_Model, only:       registerIC_Model => registerIC
...
contains
  ...
  subroutine SetServices(driver, rc)
    ...
    ! set CompLabel for self
    call NUOPC_CompAttributeSet(driver, &
      name="CompLabel", value="explorerDriver", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
#define COMPLIANCE_CHECK_DRIVER
  ...
  subroutine SetModelServices(driver, rc)
    ...
    if (trim(enable_field_mirroring)=="yes") then
      ...
      !            call ESMF_GridCompGet(driver, name=compName, rc=rc)
      !            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      !              line=__LINE__, &
      !              file=__FILE__)) &
      !              return  ! bail out
      !
      !            print *, "compName of driver = ", compName
      !
      !            call NUOPC_CompAttributeGet(driver, &
      !                name="CompLabel", value=compLabel, rc=rc)
      !            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      !              line=__LINE__, &
      !              file=__FILE__)) &
      !              return  ! bail out
      !
      !            print *, "compLabel of driver = ", compLabel
      !
      !            call NUOPC_DriverGetComp(driver, compLabel="explorerDriver", &
      !                comp=child, rc=rc)
      !            if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      !              line=__LINE__, &
      !              file=__FILE__)) &
    ...
    if (trim(enable_compliance_check)=="yes") then
      ...
      call ESMF_GridCompSetServices(child, userRoutine=registerIC_Model, &
        userRc=urc, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
    ...
    do i=0, size(compList)
      ...
      if (i > 0) then
        if (NUOPC_CompAreServicesSet(compList(i))) then
          ...
          ! step3: set the initPhase variable to stop after realizing fields
          reducedPhaseCount = 0
          phaseIdx = 1
          do k=1, phaseCount
          ...
          !                        elseif (index(trim(phaseMap(k)), "IPDv05p3") > 0) then
          !                            ! p3 valid for IPDv05
          !                            initPhase(3) = trim(phaseMap(k))
          !                        elseif (index(trim(phaseMap(k)), "IPDv05p4") > 0) then
          !                            ! p4 valid for IPDv05
          !                            initPhase(4) = trim(phaseMap(k))
          !                        endif
          enddo
          ! step4: replace the full InitializePhaseMap with reduced mappings
          !                    print *, "REDUCED phase map: ", initPhase(:)
