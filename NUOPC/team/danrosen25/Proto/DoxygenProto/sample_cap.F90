...
module sample_cap_mod
  ...
  type model_internalstate_type
    integer                 :: verbosity        = 1
    integer                 :: timeStepSeconds  = 999999999
    logical                 :: coupledForcing   = .TRUE.
    logical                 :: realizeAllExport = .FALSE.
    integer                 :: restartSeconds   = 999999999
...
#define METHOD "InitializeP0"
  ...
  subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
    ...
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out
    call ESMF_TimeIntervalGet(timeStep, s=timeStepSeconds, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out
    ...
    if (configIsPresent) then
      ...
      call ESMF_ConfigGetAttribute(config, is%wrap%timeStepSeconds, &
        label=TRIM(cname)//"_TimeStepSeconds:", &
        default=timeStepSeconds, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out

      call ESMF_ConfigGetAttribute(config, is%wrap%coupledForcing, &
        label=TRIM(cname)//"_CoupledForcing:", &
        default=is%wrap%coupledForcing, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out

      call ESMF_ConfigGetAttribute(config, is%wrap%realizeAllExport, &
        label=TRIM(cname)//"_RealizeAllExport:", &
        default=is%wrap%realizeAllExport, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out

      call ESMF_ConfigGetAttribute(config, is%wrap%restartSeconds, &
        label=TRIM(cname)//"_RestartInterval:", &
        default=is%wrap%restartSeconds, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out
      ...
      is%wrap%timeStepSeconds = ESMF_UtilString2Int(tmpStr, &
        ...
        specialValueList=(/ timeStepSeconds /), rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, file=__FILE__)) return ! bail out
      ...
      call ESMF_AttributeGet(gcomp, name="RestartInterval", value=tmpStr, &
        defaultValue="default", convention="NUOPC", purpose="Instance", &
        rc=rc)
...
#define METHOD "ModelAdvance"
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    call ESMF_ClockPrint(clock, options="currTime", &
      preString="------>Advancing "//trim(cname)//" from: ", rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, file=__FILE__)) return ! bail out
    ...
    call model_run(.NOT.is%wrap%CoupledForcing, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, file=__FILE__)) return ! bail out
