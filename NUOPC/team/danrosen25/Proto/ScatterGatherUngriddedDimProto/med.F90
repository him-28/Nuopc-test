...
module MED
  ...
  type(ESMF_State), save  :: sglFrATM, sglToATM
  type(ESMF_State), save  :: frOCN, toOCN

  type(ESMF_FieldBundle), save :: fldsFrATM, fldsToATM
  type(ESMF_FieldBundle), save :: fldsFrOCN, fldsToOCN
  type(ESMF_RouteHandle), save :: OCNtoATM
  type(ESMF_RouteHandle), save :: ATMtoOCN
  ...
  subroutine InitializeP5(mediator, importState, exportState, clock, rc)
    ...
    call createScatterState(frATM, sglFrATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call createScatterState(toATM, sglToATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out

    call createFieldBundle(sglFrATM, fldsFrATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call createFieldBundle(sglToATM, fldsToATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call createFieldBundle(frOCN, fldsFrOCN, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call createFieldBundle(toOCN, fldsToOCN, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
#ifdef REGRID_on
    call ESMF_FieldBundleRegridStore(fldsFrATM, fldsToOCN, &
      routehandle=ATMtoOCN, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_FieldBundleRegridStore(fldsFrOCN, fldsToATM, &
      routehandle=OCNtoATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    subroutine createScatterState(state, newstate, rc)
      ...
      ! create empty state
      newstate = ESMF_StateCreate(rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
      ...
      do item=1, itemCount
        if (itemTypeList(item)==ESMF_STATEITEM_FIELD) then
          ...
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            ...
            file=__FILE__)) &
            return  ! bail out
          newfield = ESMF_FieldCreate(name=trim(itemNameList(item)), &
            grid=grid, typekind=ESMF_TYPEKIND_R8, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
          call ESMF_StateAdd(newstate, fieldList=(/newfield/), rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
    ...
    subroutine createFieldBundle(state, fb, rc)
      ! Look at all of the fields in state, including in nested states. Create
      ! field bundle in alphabeitcal order
      type(ESMF_State)       :: state
  ...
  subroutine DataInitialize(mediator, rc)
    ...
    ! fill toATM state with -1
    call ESMF_StateGet(toATM, nestedFlag=.true., itemCount=itemCount, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    allocate(itemNameList(itemCount), itemTypeList(itemCount))
    call ESMF_StateGet(toATM, nestedFlag=.true., &
      itemNameList=itemNameList, itemTypeList=itemTypeList, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    do item=1, itemCount
      if (itemTypeList(item)==ESMF_STATEITEM_FIELD) then
        call ESMF_StateGet(toATM, field=field, itemName=itemNameList(item), &
          rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
        call ESMF_FieldFill(field, dataFillScheme="const", const1=-1.0D0, rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
          return  ! bail out
    ...
    deallocate(itemNameList, itemTypeList)
     ...
     ! fill toOCN state with ones
    call ESMF_StateGet(toOCN, nestedFlag=.true., itemCount=itemCount, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    allocate(itemNameList(itemCount), itemTypeList(itemCount))
    call ESMF_StateGet(toOCN, nestedFlag=.true., &
      itemNameList=itemNameList, itemTypeList=itemTypeList, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    do item=1, itemCount
      if (itemTypeList(item)==ESMF_STATEITEM_FIELD) then
        call ESMF_StateGet(toOCN, field=field, itemName=itemNameList(item), &
          rc=rc)
        if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
          line=__LINE__, &
          file=__FILE__)) &
  ...
  subroutine MediatorAdvance(mediator, rc)
    ...
    call ESMF_StateGet(frATM, itemCount=itemCount(1), rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out

    call ESMF_StateGet(toATM, itemCount=itemCount(2), rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out

    call ESMF_StateGet(frOCN, itemCount=itemCount(3), rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out

    call ESMF_StateGet(toOCN, itemCount=itemCount(4), rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    write (msgString,*) "item counts for: "// &
      "frATM, toATM, frOCN, toOCN:", itemCount
    call ESMF_LogWrite(msgString, ESMF_LOGMSG_INFO, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    call copyFrEnsState(frATM, sglFrATM, slice=1, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
...
#ifdef REGRID_on
    call ESMF_FieldBundleRegrid(fldsFrATM, fldsToOCN, &
      routehandle=ATMtoOCN, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    call ESMF_FieldBundleRegrid(fldsFrOCN, fldsToATM, &
      routehandle=OCNtoATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    call copyToEnsState(sglToATM, toATM, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    subroutine copyFrEnsState(ensState, toState, slice, rc)
      ...
      type(ESMF_Field)                        :: toField
#ifdef  UNGRIDDEDDIM_on
      real(ESMF_KIND_R8), pointer             :: ensFarrayPtr(:,:,:)
#else
      real(ESMF_KIND_R8), pointer             :: ensFarrayPtr(:,:)
#endif
      real(ESMF_KIND_R8), pointer             :: toFarrayPtr(:,:)
      ...
      do i=1, itemCount
        if (itemTypeList(i)==ESMF_STATEITEM_FIELD) then
          call ESMF_StateGet(ensState, field=ensField, &
          ...
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            ...
            file=__FILE__)) &
            return  ! bail out
          call ESMF_StateGet(toState, field=toField, &
            itemName=itemNameList(i), rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
          call ESMF_FieldGet(toField, farrayPtr=toFarrayPtr, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
#ifdef  UNGRIDDEDDIM_on
          toFarrayPtr(:,:) = ensFarrayPtr(:,:,slice)
#else
          toFarrayPtr(:,:) = ensFarrayPtr(:,:)
#endif
        endif
      enddo
    ...
    subroutine copyToEnsState(frState, ensState, slice, rc)
      type(ESMF_State)       :: frState
      type(ESMF_State)       :: ensState
      integer, optional      :: slice
      integer, optional      :: rc
      ! local variables
      integer                                 :: itemCount, i, j
      integer                                 :: fieldCount
      character(len=80), allocatable          :: itemNameList(:)
      type(ESMF_StateItem_Flag), allocatable  :: itemTypeList(:)
      type(ESMF_Field)                        :: ensField
      type(ESMF_Field)                        :: frField
#ifdef  UNGRIDDEDDIM_on
      real(ESMF_KIND_R8), pointer             :: ensFarrayPtr(:,:,:)
#else
      real(ESMF_KIND_R8), pointer             :: ensFarrayPtr(:,:)
#endif
      real(ESMF_KIND_R8), pointer             :: frFarrayPtr(:,:)

      if (present(rc)) rc = ESMF_SUCCESS

      ! query info about the items in the state
      call ESMF_StateGet(frState, itemCount=itemCount, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
        line=__LINE__, &
        file=__FILE__)) &
        return  ! bail out
      allocate(itemNameList(itemCount), itemTypeList(itemCount))
      call ESMF_StateGet(frState, itemNameList=itemNameList, &
      ...
      do i=1, itemCount
        if (itemTypeList(i)==ESMF_STATEITEM_FIELD) then
          call ESMF_StateGet(frState, field=frField, &
            itemName=itemNameList(i), rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
          call ESMF_FieldGet(frField, farrayPtr=frFarrayPtr, rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
          ...
          if (present(slice)) then
            ensFarrayPtr(:,:,slice) = frFarrayPtr(:,:)
          else
            do j=lbound(ensFarrayPtr,3), ubound(ensFarrayPtr,3)
              ensFarrayPtr(:,:,j) = frFarrayPtr(:,:)
            enddo
          endif
#else
          ensFarrayPtr(:,:) = frFarrayPtr(:,:)
#endif
        endif
      enddo
