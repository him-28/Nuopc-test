module NUOPC_Model_Noah

    !-----------------------------------------------------------------------------
    ! NUOPC Noah Model Component
    !-----------------------------------------------------------------------------
    ...
    !
    ! MODULE_IO
    ! Accesses most of our top-level input/output routines.  This
    ...
    ! Contains a few utility routines needed for setting up some data for SFLX.
    ! This driver program makes use of module subroutines CALTMP and CALHUM.
    !
    ! MODULE_SF_NOAHLSM
    ! Contains the Noah LSM physics code.  This driver program
    ! makes use of module subroutines SFLX, SOIL_VEG_GEN_PARM, and SFCDIF_off.
    !
    ! MODULE_SF_NOAHLSM_GLACIAL_ONLY
    ! Contains the Noah LSM physics code that applies to
    ! glacial points.  This driver program makes use of module subroutine SFLX_GLACIAL.
    !
    ! MODULE_SFCDIF_WRF
    ! Contains the MYJSFC version of SFCDIF.  This driver program
    ! makes use of subroutines MYJSFCINIT and SFCDIF_MYJ.
    !
    ! KWM_DATE_UTILITIES
    ! Contains handy subroutines for manipulating and computing date/time information.
    !-----------------------------------------------------------------------------

    use module_io, only: &
        output_close, &
            ! This subroutine closes ascii and netcdf output units
        initialize_output, &
            ! This subroutine opens ascii and netcdf output files.
        output_time, &
            ! This subroutine writes date/time to ascii and netcdf units
        output_levels, &
            ! This subroutine writes out fields that are dimensioned by NLEVELS
            ! to ascii and netcdf output files.
        output_var, &
            ! This subroutine writes out individual (single-layer) values to
            ! ascii and netcdf output files.
        finish_output_for_time, &
            ! This subroutine does some cleans-up tasks for output at the end
            ! of each output time.  At the moment, nothing needs do be done
            ! here for the NetCDF output.
        open_forcing_file, &
            ! This subroutine opens an initial/forcing conditions file for input.
            ...
            ! This subroutine is contained in module_ascii_io.F
        read_forcing_text, &
            ! This subroutine reads the forcing data for a single time step from
            ! an open ASCII text file attached to Fortran unit <iunit>.
            ! This suddenly got a lot more complicated, now that we need to allow
            ! for temporal interpolation to a shorter timestep than is available
            ! in the forcing dataset.
            ! This subroutine is contained in module_ascii_io.F
        badval
            ! This parameter of type real is set to -1.E36 in module_ascii_io.F
    use module_noahlsm_utility, only: &
        CALTMP, &
            ! This subroutine outputs Potential temperature (considering the
            ! reference pressure to be at the surface), Virtual skin temperature (K).
            ! Virtual potential temperature at ZLVL, and Density
        CALHUM
            ! This subroutine outputs Saturated specific humidity and DQSDT2
    use module_sf_noahlsm, only: &
        sflx, &
            ! This subroutine is the "Noah LSM" (SOIL/VEG/SNOWPACK) physics driver to update
            ! SOIL MOISTURE, SOIL ICE, SOIL TEMPERATURE, SKIN TEMPERATURE,
            ! SNOWPACK WATER CONTENT, SNOWDEPTH, and all terms of the surface
            ! energy balance and surface water balance
        SFCDIF_OFF, &
            ! This subroutine calculates surface layer exchange coefficients via
            ! an interative process. See Chen et al (1997,BLM)
            ! If _SIMPLE_DRIVER_ is defined then this subroutine includes
            ! VEGTYP, ISURBAN, IZ0TLND, ZLM_WIND
        stbolt, &
            ! This parameter of type real is set to 5.67051E-8 in module_model_constants.F
        CZIL_DATA, &
            ! This variable of type real calculate roughness length of heat
        ALBEDOMAXTBL, &
            ! This variable array (50 elem) of type real is used in vegetation parameters
        Z0MINTBL
            ! This variable array (50 elem) of type real is used in vegetation parameters
    use module_sf_noahlsm_glacial_only, only: &
        SFLX_GLACIAL
            ! This subroutine is the "Noah LSM" (SOIL/VEG/SNOWPACK) physics driver to update
            ! ICE TEMPERATURE, SKIN TEMPERATURE, SNOWPACK WATER CONTENT, SNOWDEPTH,
            ! and all terms of the surface energy balance
    use module_sfcdif_wrf, only: &
        MYJSFCINIT, &
            ! This subroutine initializes dzeta2, ztmax2, PSIH2, and PSIM2 in
            ! module_sfcdif_wrf
        SFCDIF_MYJ
        ! This subroutine outputs AKHS, AKMS, RIB
    use kwm_date_utilities, only: &
        geth_newdate
            ! This subroutine changes date by idt in hdate format 'YYYY-MM-DD HH:MM:SS.ffff'

        !-----------------------------------------------------------------------------
        ! Subroutines contained in module_Noahlsm_utility.F
        ! These subroutines are not contained in module_Noahlsm_utility.mod
        ! Therefore NUOPC_Model_Noah requires module_Noahlsm_utility.o and
        ! symbol conflicts must be considered.
        !  - wrf_error_fatal: prints string and stops execution
        !  - wrf_message: prints string
        !  - wrf_dm_bcast_string: does nothing
        !  - wrf_dm_bcast_integer: does nothing
        !  - wrf_dm_bcast_real: does nothing
        !-----------------------------------------------------------------------------
    ...
    !-----------------------------------------------------------------------------
    ! Noah LSM variable declarations
    !-----------------------------------------------------------------------------

    ! Command-line:
    character(len=256) :: executable_name   ! Executable name from argument 0
    character(len=256) :: forcing_filename  ! Forcing condition filename from argument 1
    ...
    character(len=4096) :: infotext         ! Useful user information
    ...
    !                                      ! stop the execution.
    !real, external     :: month_d         ! Function included in this module
...
!-----------------------------------------------------------------------------
! End Noah LSM variable Declaration
!-----------------------------------------------------------------------------
...
#define WITHIMPORTFIELDS___disable
    ...
    subroutine NoahTimeLoop()
        integer :: complete,err

        TIMELOOP : do while ( nowdate < enddate)
            call NoahAdvance(complete,err)
            if(err == 1) then
                exit TIMELOOP
                stop ":  FORCING DATA READ PROBLEM"
            else if(complete == 1) then
                exit TIMELOOP
            end if
        enddo TIMELOOP
    end subroutine

    subroutine NoahAdvance(complete,err)
        integer,intent(out) :: complete, err
        ...
        if ( ( loop_for_a_while > 0 ) .and. ( nowdate == enddate ) ) then
            ...
            if ( reloop_count >= loop_for_a_while ) then
                complete = 1
                return
            end if
...
2003 CONTINUE
 ...
 subroutine DebugPrint(heading,linelabel)
     character(len=*),intent(in) :: heading
     character,intent(in)        :: linelabel

     print*, heading
     print*, linelabel,'  FFROZP = ', FFROZP
     print*, linelabel,'  ICE = ', ICE
            !              print*, linelabel,'  ISURBAN = ', ISURBAN
     print*, linelabel,'  DT = ', DT
     print*, linelabel,'  ZLVL = ', ZLVL
     print*, linelabel,'  NSOIL = ', NSOIL
     print*, linelabel,'  SLDPTH = ', SLDPTH
     print*, linelabel,'  LLANDUSE = ', trim(LLANDUSE)
     print*, linelabel,'  LSOIL = ', trim(LSOIL)
     print*, linelabel,'  LWDN = ', LWDN
     print*, linelabel,'  SOLDN = ', SOLDN
     print*, linelabel,'  SOLNET = ', SOLNET
     print*, linelabel,'  SFCPRS = ', SFCPRS
     print*, linelabel,'  PRCP = ', PRCP
     print*, linelabel,'  SFCTMP = ', SFCTMP
     print*, linelabel,'  Q2 = ', Q2
     print*, linelabel,'  SFCSPD = ', SFCSPD
     print*, linelabel,'  COSZ = ', COSZ
     print*, linelabel,'  PRCPRAIN = ', PRCPRAIN
     print*, linelabel,'  SOLARDIRECT = ', SOLARDIRECT
     print*, linelabel,'  TH2 = ', TH2
     print*, linelabel,'  Q2SAT = ', Q2SAT
     print*, linelabel,'  DQSDT2 = ', DQSDT2
     print*, linelabel,'  VEGTYP = ', VEGTYP
     print*, linelabel,'  SOILTYP = ', SOILTYP
     print*, linelabel,'  SLOPETYP = ', SLOPETYP
     print*, linelabel,'  SHDFAC = ', SHDFAC
            !              print*, linelabel,'  SHDMIN = ', SHDMIN
            !              print*, linelabel,'  SHDMAX = ', SHDMAX
            !              print*, linelabel,'  ALB = ', ALB
     print*, linelabel,'  SNOALB = ', SNOALB
     print*, linelabel,'  TBOT = ', TBOT
     print*, linelabel,'  Z0BRD = ', Z0BRD
     print*, linelabel,'  Z0 = ', Z0
     print*, linelabel,'  EMISSI = ', EMISSI
            !              print*, linelabel,'  EMBRD = ', EMBRD
     print*, linelabel,'  CMC = ', CMC
     print*, linelabel,'  T1 = ', T1
     print*, linelabel,'  STC = ', STC
     print*, linelabel,'  SMC = ', SMC
     print*, linelabel,'  SH2O = ', SH2O
     print*, linelabel,'  SNOWH = ', SNOWH
     print*, linelabel,'  SNEQV = ', SNEQV
     print*, linelabel,'  ALBEDO = ', ALBEDO
     print*, linelabel,'  CH = ', CH
     print*, linelabel,'  CM = ', CM
     print*, linelabel,'  ETA = ', ETA
     print*, linelabel,'  SHEAT = ', SHEAT
     print*, linelabel,'  ETAKIN = ', ETAKIN
     print*, linelabel,'  FDOWN = ', FDOWN
     print*, linelabel,'  EC = ', EC
     print*, linelabel,'  EDIR = ', EDIR
     print*, linelabel,'  ET = ', ET
     print*, linelabel,'  ETT = ', ETT
     print*, linelabel,'  ESNOW = ', ESNOW
     print*, linelabel,'  DRIP = ', DRIP
     print*, linelabel,'  DEW = ', DEW
     print*, linelabel,'  BETA = ', BETA
     print*, linelabel,'  ETP = ', ETP
     print*, linelabel,'  SSOIL = ', SSOIL
     print*, linelabel,'  FLX1 = ', FLX1
     print*, linelabel,'  FLX2 = ', FLX2
     print*, linelabel,'  FLX3 = ', FLX3
     print*, linelabel,'  SNOMLT = ', SNOMLT
     print*, linelabel,'  SNCOVR = ', SNCOVR
     print*, linelabel,'  RUNOFF1 = ', RUNOFF1
     print*, linelabel,'  RUNOFF2 = ', RUNOFF2
     print*, linelabel,'  RUNOFF3 = ', RUNOFF3
     print*, linelabel,'  RC = ', RC
     print*, linelabel,'  PC = ', PC
     print*, linelabel,'  RSMIN = ', RSMIN
            !              write(*, '("XLAI = ", F7.5)') XLAI
     print*, linelabel,'  RCS = ', RCS
     print*, linelabel,'  RCT = ', RCT
     print*, linelabel,'  RCQ = ', RCQ
     print*, linelabel,'  RCSOIL = ', RCSOIL
     print*, linelabel,'  SOILW = ', SOILW
     print*, linelabel,'  SOILM = ', SOILM
     print*, linelabel,'  Q1 = ', Q1
#if 0
     print*, linelabel,'  RDLAI2D = ', RDLAI2D
     print*, linelabel,'  USEMONALB = ', USEMONALB
     print*, linelabel,'  SNOTIME1 = ', SNOTIME1
     print*, linelabel,'  RIBB = ', RIBB
#endif
     print*, linelabel,'  SMCWLT = ', SMCWLT
     print*, linelabel,'  SMCDRY = ', SMCDRY
     print*, linelabel,'  SMCREF = ', SMCREF
     print*, linelabel,'  SMCMAX = ', SMCMAX
     print*, linelabel,'  NROOT = ', NROOT
 end subroutine
