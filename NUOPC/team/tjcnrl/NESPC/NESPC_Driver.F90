!-------------------------------------------------------------------------------
! NAVY ESPC Driver Component
!-------------------------------------------------------------------------------
! CONFIGURATION IDENTIFICATION $HeadURL$
! CONFIGURATION IDENTIFICATION @(#)$Id$
!-------------------------------------------------------------------------------
#define FILENAME "NESPC_Driver.F90"
#include "NESPC_Macros.h"

!-------------------------------------------------------------------------------
! Default included component modules
!-------------------------------------------------------------------------------
#define FRONT_DUMB NESPC_Mdumb
#define FRONT_CON  NESPC_Connector
#undef  FRONT_MED

#define FRONT_ATM0 NESPC_Mdata
#define  TYPE_ATM0 "data"
#define FRONT_OCN0 NESPC_Mdata
#define  TYPE_OCN0 "data"
#define FRONT_WAV0 NESPC_Mdata
#define  TYPE_WAV0 "data"
#define FRONT_ICE0 NESPC_Mdata
...
! use NUOPC_Driver, parent_SetServices => SetServices
  ...
  subroutine SetModelServices(driver, rc)
    ...
    ! process config to set modTndx for models
    ...
    ! define NESPC component level attributes
    attrList(1) = 'MissingValue'
    attrList(2) = 'ConnectorType' ! 'memcpy', 'redist', 'bilinr', or 'bicubc'
    ...
    do i = 1,modCount
      ...
      ! add NESPC Attribute package
      call ESMF_AttributeAdd(modComp(i), convention='NESPC', purpose='General', &
      ...
      ! set NESPC Attributes
      call ESMF_AttributeSet(modComp(i), name='MissingValue', &
        value=missingValue, convention='NESPC', purpose='General', rc=rc)
      if (ESMF_LogFoundError(rc, PASSTHRU)) then
    ...
    do i = 1,modCount
      ...
      ! add NESPC Attribute package
      call ESMF_AttributeAdd(conComp(i,j), convention='NESPC', purpose='General', &
        attrList=attrList, rc=rc)
      if (ESMF_LogFoundError(rc, PASSTHRU)) then
      ...
      call ESMF_AttributeSet(conComp(i,j), name='ConnectorType', &
        value=conType(i,j), convention='NESPC', purpose='General', rc=rc)
      if (ESMF_LogFoundError(rc, PASSTHRU)) then
        write(msgString,'(a,2i2,a)') 'ESMF_AttributeSet: ',i,j,', '//conName(i,j)
        call ESMF_LogWrite(trim(msgString), ESMF_LOGMSG_ERROR)
        return ! bail out
