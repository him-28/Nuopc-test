!-------------------------------------------------------------------------------
! NAVY ESPC 4dvar application driver
!-------------------------------------------------------------------------------
! CONFIGURATION IDENTIFICATION $HeadURL$
! CONFIGURATION IDENTIFICATION @(#)$Id$
!-------------------------------------------------------------------------------
#define FILENAME "NESPC_VdaDriver.F90"
...
program mainApp
  ...
  gcomp_ad = ESMF_GridCompCreate(name='NESPC_AD', rc=rc)
  if (ESMF_LogFoundError(rc, PASSTHRU)) call ESMF_Finalize(endflag=ESMF_END_ABORT)
  gcomp_rp = ESMF_GridCompCreate(name='NESPC_RP', rc=rc)
  if (ESMF_LogFoundError(rc, PASSTHRU)) call ESMF_Finalize(endflag=ESMF_END_ABORT)
