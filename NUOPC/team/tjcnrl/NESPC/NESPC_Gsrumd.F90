!-------------------------------------------------------------------------------
! NAVY ESPC Grid-Search-Remapping Utilities Module (derived from WW3 w3gsrumd.ftn)
!-------------------------------------------------------------------------------
! CONFIGURATION IDENTIFICATION $HeadURL$
! CONFIGURATION IDENTIFICATION @(#)$Id$
!-------------------------------------------------------------------------------
#define FILENAME "NESPC_Gsrumd.F90"
...
!/
      ...
      INTERFACE FINDIJ
        MODULE PROCEDURE FINDIJ_R4
        MODULE PROCEDURE FINDIJ_R8
      END INTERFACE FINDIJ
      INTERFACE W3GFIJ
...
!
      ...
      IF ( ICLO.NE.ICLO_NONE .AND. (LBX.NE.1 .OR. LBY.NE.1) ) THEN
          WRITE(*,'(/1A,1A/)') 'W3GSUC_R4 ERROR -- ', &
          'index space closure with grid index lower-bounds != 1 is not supported'
          CALL EXTCDE (1)
        END IF
...
!/ ------------------------------------------------------------------- /
      FUNCTION FINDIJ_R4(GSU, XTIN, YTIN, IX, JX, DEBUG) RESULT(INGRID)
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!/
      LOGICAL :: LDBG, POLE
      INTEGER :: IS(4), JS(4)
      REAL(4) :: XT, YT, XS(4), YS(4), DD
      INTEGER :: IS1(4), JS1(4)
      REAL(4) :: XS1(4), YS1(4), DD1
      INTEGER :: I, J, K, L, N, IB, JB
      LOGICAL :: IJG, LLG, LCLO, L360
      INTEGER :: ICLO, GKIND
      INTEGER :: LBX, LBY, UBX, UBY, NX, NY
...
!/
!/S      INTEGER, SAVE           :: IENT = 0
!/S      CALL STRACE (IENT, 'FINDIJ_R4')
!
! -------------------------------------------------------------------- /
! 1.  Test input
!
      IF ( .NOT.ASSOCIATED(GSU%PTR) ) THEN
          WRITE(*,'(/2A/)') 'FINDIJ_R4 ERROR -- ', &
          'grid search utility object not created'
          CALL EXTCDE (1)
        END IF
!
...
!
      XT = XTIN;  YT = YTIN;
      IF ( LDBG ) WRITE(*,'(/A,2E14.6)') 'FINDIJ_R4 - TARGET POINT:',XT,YT
!
! -------------------------------------------------------------------- /
! 3.  Find enclosing or closest cell and compute point location
!
      INGRID = W3GFCL(GSU,XT,YT,IS,JS,XS,YS,POLE=POLE,DEBUG=LDBG)
      IF ( .NOT. INGRID ) THEN
      POLE = .FALSE.
      DD1 = HUGE(XT)
!-----closest search bucket
      IB = INT((XT-XMIN)/DXB)+1
      IB = MAX(1,MIN(IB,NBX))
      JB = INT((YT-YMIN)/DYB)+1
      JB = MAX(1,MIN(JB,NBY))
      CELL_LOOP: DO K=1,B(JB,IB)%N
!---------setup cell corner indices
          IS(1) = B(JB,IB)%I(K)  ;  JS(1) = B(JB,IB)%J(K)  ;
...
!---------non-pole cell: compute relative location
          ...
          ! set in grid if point is within 1 cell width distance of closest cell
          INGRID = (IX.GE.-ONE.AND.IX.LE.TWO.AND.JX.GE.-ONE.AND.JX.LE.TWO)
          ! set absolute index space location
...
!/ End of FINDIJ_R4 -------------------------------------------------- /
!/
      END FUNCTION FINDIJ_R4
!/ ------------------------------------------------------------------- /
      FUNCTION FINDIJ_R8(GSU, XTIN, YTIN, IX, JX, DEBUG) RESULT(INGRID)
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
!/                  |        T. J. Campbell, NRL        |
...
!/
      LOGICAL :: LDBG, POLE
      INTEGER :: IS(4), JS(4)
      REAL(8) :: XT, YT, XS(4), YS(4), DD
      INTEGER :: IS1(4), JS1(4)
      REAL(8) :: XS1(4), YS1(4), DD1
      INTEGER :: I, J, K, L, N, IB, JB
      LOGICAL :: IJG, LLG, LCLO, L360
      INTEGER :: ICLO, GKIND
      INTEGER :: LBX, LBY, UBX, UBY, NX, NY
...
!/
!/S      INTEGER, SAVE           :: IENT = 0
!/S      CALL STRACE (IENT, 'FINDIJ_R8')
!
! -------------------------------------------------------------------- /
! 1.  Test input
!
      IF ( .NOT.ASSOCIATED(GSU%PTR) ) THEN
          WRITE(*,'(/2A/)') 'FINDIJ_R8 ERROR -- ', &
          'grid search utility object not created'
          CALL EXTCDE (1)
        END IF
!
...
!
      XT = XTIN;  YT = YTIN;
      IF ( LDBG ) WRITE(*,'(/A,2E14.6)') 'FINDIJ_R8 - TARGET POINT:',XT,YT
!
! -------------------------------------------------------------------- /
