module ESM
  ...
  subroutine SetServices(gcomp, rc)
    ...
    ! NUOPC_DriverCOAMPS registers the generic methods
  ...
  subroutine SetModelServices(gcomp, rc)
    ...
    ! SetServices for wav
    call ESMF_GridCompSetServices(is%wrap%wav, wavSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    ! SetServices for med2wav
    call ESMF_CplCompSetServices(is%wrap%med2wav, cplSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    ! SetServices for ocn2wav
    call ESMF_CplCompSetServices(is%wrap%ocn2wav, cplSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
    ...
    ! SetServices for wav2ocn
    call ESMF_CplCompSetServices(is%wrap%wav2ocn, cplSS, userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
      return  ! bail out
