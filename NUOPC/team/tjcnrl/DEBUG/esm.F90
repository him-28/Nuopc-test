module ESM
  ...
  subroutine SetModelCount(gcomp, rc)
    ...
    ! set the modelCount for ATM-OCN-WAV coupling
...
#define PETLAYOUT 1
  ...
  subroutine SetModelServices(gcomp, rc)
    ...
    ! SetServices for WAV -> as modelComp(3)
    call ESMF_GridCompSetServices(is%wrap%modelComp(3), wavSS, &
      userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
    ...
    call ESMF_CplCompSetServices(is%wrap%connectorComp(2,3), cplSS, &
      userRc=localrc, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, &
      file=__FILE__)) &
