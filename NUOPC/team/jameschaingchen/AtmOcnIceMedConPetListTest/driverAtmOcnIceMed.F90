...
module DriverAtmOcnIceMed

  !-----------------------------------------------------------------------------
  ! Generic Driver Component for ATM, OCN, ICE, MED with default explicit time stepping
  !-----------------------------------------------------------------------------
  ...
  character(*), parameter :: &
    label_InternalState = "DriverAtmOcnIceMed_InternalState"
  character(*), parameter :: &
    label_SetModelPetLists = "DriverAtmOcnIceMed_SetModelPetLists"
  character(*), parameter :: &
    label_SetModelServices = "DriverAtmOcnIceMed_SetModelServices"
  character(*), parameter :: &
    label_Finalize = "DriverAtmOcnIceMed_Finalize"
  ...
  subroutine SetModelPetLists(gcomp, rc)
    ...
    nullify(is%wrap%ice2medPetList)
    nullify(is%wrap%med2atmPetList)
    nullify(is%wrap%med2ocnPetList)
    nullify(is%wrap%med2icePetList)
    ...
    if (existflag) then
      ...
      superIS%wrap%modelPetLists(3)%petList => is%wrap%icePetList
      superIS%wrap%modelPetLists(4)%petList => is%wrap%medPetList
      superIS%wrap%connectorPetLists(1,4)%petList => is%wrap%atm2medPetList
      superIS%wrap%connectorPetLists(2,4)%petList => is%wrap%ocn2medPetList
      superIS%wrap%connectorPetLists(3,4)%petList => is%wrap%ice2medPetList
      superIS%wrap%connectorPetLists(4,1)%petList => is%wrap%med2atmPetList
      superIS%wrap%connectorPetLists(4,2)%petList => is%wrap%med2ocnPetList
      superIS%wrap%connectorPetLists(4,3)%petList => is%wrap%med2icePetList
    endif
  ...
  subroutine SetModelServices(gcomp, rc)
    ...
    is%wrap%ice = superIS%wrap%modelComp(3)
    is%wrap%iceIS = superIS%wrap%modelIS(3)
    is%wrap%iceES = superIS%wrap%modelES(3)

    is%wrap%med = superIS%wrap%modelComp(4)
    is%wrap%medIS = superIS%wrap%modelIS(4)
    is%wrap%medES = superIS%wrap%modelES(4)

    is%wrap%atm2med = superIS%wrap%connectorComp(1,4)
    is%wrap%ocn2med = superIS%wrap%connectorComp(2,4)
    is%wrap%ice2med = superIS%wrap%connectorComp(3,4)

    is%wrap%med2atm = superIS%wrap%connectorComp(4,1)
    is%wrap%med2ocn = superIS%wrap%connectorComp(4,2)
    is%wrap%med2ice = superIS%wrap%connectorComp(4,3)
    ...
    ! suitable for the ATM-OCN-ICE-MED case. The default RunSeq must be overwritten.
    ...
    ! ocn2med in runSeq(1)
    call NUOPC_RunElementAdd(superIS%wrap%runSeq(1), i=2, j=4, phase=1, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, file=__FILE__)) return  ! bail out
...
!      line=__LINE__, file=__FILE__)) return  ! bail out
    ...
    ! med2ocn in runSeq(1)
    call NUOPC_RunElementAdd(superIS%wrap%runSeq(1), i=4, j=2, phase=1, rc=rc)
    if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
      line=__LINE__, file=__FILE__)) return  ! bail out
